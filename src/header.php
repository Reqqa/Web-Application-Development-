...
<nav class="custom-navbar" style="background: #0a0a0a; border-bottom: 1px solid rgba(255,255,255,0.1); padding: 16px 40px; display: flex; align-items: center; justify-content: space-between; font-family: sans-serif;">
    <div class="nav-branding" style="display: flex; align-items: center; gap: 12px;">
        <a href="index.php" style="color: #ffffff; text-decoration: none; font-size: 1.3rem; font-weight: 700; letter-spacing: -0.5px;">Course<span style="color: #ffffff; opacity: 0.7;">Map</span></a>
    </div>
    <div class="nav-auth-actions" style="display: flex; align-items: center; gap: 16px;">
        <?php if (isset($_SESSION["user_id"])): ?>
            ...
            <a onclick="return confirm('are you sure you want to logout?')";    href="logout.php" style="color: #ffffff; border: 1px solid rgba(255,255,255,0.25); text-decoration: none; padding: 8px 18px; border-radius: 20px; font-size: 0.85rem; font-weight: 600; background: transparent; transition: background 0.2s;">
                Sign out
            </a>
        <?php else: ?>
